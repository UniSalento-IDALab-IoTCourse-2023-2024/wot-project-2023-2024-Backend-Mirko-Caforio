...
import it.unisalento.iotproject.notificationservice.domain.NotificationFactory;
import it.unisalento.iotproject.notificationservice.domain.PopupNotification;
import it.unisalento.iotproject.notificationservice.dto.*;
...
public class NotificationService {
    ...
    public List<PopupNotification> findPopupNotifications(NotificationQueryFilters filters) {
        Query query = new Query();

        // Add conditions based on parameters provided
        if (filters.getEmail() != null) {
            query.addCriteria(Criteria.where("email").is(filters.getEmail()));
        }

        if (filters.getSubject() != null) {
            query.addCriteria(Criteria.where("subject").is(filters.getSubject()));
        }
        if (filters.getFrom() != null) {
            query.addCriteria(Criteria.where("sendAt").gte(filters.getFrom()));
        }
        if (filters.getTo() != null) {
            query.addCriteria(Criteria.where("sendAt").lte(filters.getTo()));
        }

        if (filters.getRead() != null) {
            query.addCriteria(Criteria.where("read").is(filters.getRead()));
        }
