...
import static it.unisalento.iotproject.notificationservice.security.SecurityConstants.ROLE_ADMIN;
...
public class UserCheckService {
    ...
    @Value("${mqtt.notification.auth.response.topic}")
    private String notificationAuthResponseTopic;
    ...
    public UserDetailsDTO loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        try {
            user = mosquittoExchanger.exchangeMessage(email, cqrsAuthRequestTopic, notificationAuthResponseTopic, 2000);
        } catch (Exception e) {
